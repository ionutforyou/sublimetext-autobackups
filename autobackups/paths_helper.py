...
class PathsHelper(object):
    ...
    def timestamp_file(filename):
        ...
        if backup_per_day:
            if backup_per_time == 'file':
                name = '%s_%s_%s%s' % (filepart, date, time,
                        extensionpart)
            if backup_per_time == 'folder':
                name = '%s/%s_%s%s' % (date, filepart, time,
                        extensionpart)
        else:
            if backup_per_time == 'file':
                name = '%s_%s%s' % (filepart, time, extensionpart)
            if backup_per_time == 'folder':
                name = '%s/%s%s' % (time, filepart, extensionpart)
    ...
    def get_backup_filepath(filepath):
        ...
        return os.path.join(PathsHelper.get_backup_path(filepath),
                            PathsHelper.timestamp_file(filename))
